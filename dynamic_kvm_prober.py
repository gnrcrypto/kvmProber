...
def probe(dev_path):
    ...
    # Example: Try KVM_CREATE_VM (0xAE01), KVM_GET_API_VERSION (0xAE00)
    try:
        with open(dev_path, 'rb+', buffering=0) as f:
...
def probe(dev_path):
    ...
    # /dev/vfio is a directory, so skip probing if not a file
    import os
...
def probe(dev_path):
    ...
    # /dev/net is a directory, so skip probing if not a file
    import os
    if not os.path.isfile(dev_path):
        print(f"  [!] {dev_path} is not a file, skipping.")
        return
