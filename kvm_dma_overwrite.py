...
IOCTL_WRITE_VQ_DESC_DEF = "_IOW('k', 0x32, struct vq_desc_user_data)" # For later
...
def setup_kvm_probe_lab():
    ...
    print("--- [KVM Probe Lab Setup - Virtqueue Allocation Test (v3 - Full Escape Review)] ---")
...
"""
    ...
    try:
        ...
        print(f"--- Test new VQ commands (use new prober path): ---")
        print(f"  sudo {user_prober_exe_path} allocvqpage")
        print(f"  # (Note the PFN, then try to write a simple descriptor)")
        print(f"  sudo {user_prober_exe_path} writevqdesc 0 0xYOUR_BUFFER_GPA 64 0x1 1 # Example: desc 0, buf GPA, len 64, FLAGS_NEXT, next_idx 1")
        print(f"  sudo {user_prober_exe_path} freevqpage")
        print(f"---")
        print(f"To cleanup when done:")
        print(f"  sudo rmmod {MODULE_NAME}")
        print(f"  sudo rm -rf {workdir}")
        print("---")
        ...
        # Optional: auto-unload, but for testing better to leave it loaded and give manual cleanup.
        # if module_loaded_successfully and workdir and os.path.exists(workdir) and auto_cleanup:
        #     print(f"🧹 Auto-unloading module and cleaning up {workdir}...")
        #     run_command(["sudo", "rmmod", MODULE_NAME], ignore_errors=True, check=False, cwd=workdir)
        #     shutil.rmtree(workdir)
        # el
        if not module_loaded_successfully and workdir and os.path.exists(kernel_module_ko_path):
             print(f"ℹ️ Module '{MODULE_NAME}' might not have loaded successfully.")
        pass
